...
def main():
    ...
    frontend_dir = os.path.join(base_dir, "frontend") # We run from root 'frontend' now that turbo is fixed
    ...
    if os.name == 'nt':
        # 'start' command opens a new command prompt
        # '/k' keeps the window open after the command runs or if it crashes
        subprocess.Popen(
