...
from services.conversation_service import format_messages_for_ollama, add_message_to_conversation
from services.ollama_service import generate_chat_response_stream
...
async def list_conversations(current_user: dict = Depends(get_current_user)):
    conversations = await conversations_collection.find({"user_id": current_user["_id"]}).to_list(100)
...
async def chat(conversation_id: str, request_data: ChatRequest, req: Request, current_user: dict = Depends(get_current_user)):
    ...
    # Get all messages formatted for ollama
    messages_for_ollama, model_name = await format_messages_for_ollama(conversation_id)
    
    async def stream_generator():
        full_response = ""
        try:
            async for text_chunk in generate_chat_response_stream(messages_for_ollama, model=model_name):
